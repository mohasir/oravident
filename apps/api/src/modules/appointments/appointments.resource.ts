...
export interface AppointmentJoined {
  ...
  clinic: ClinicSelect | null;
  branch: BranchSelect | null;
...
export const appointmentWithRelationsResource = (data: AppointmentJoined) => {
  const { appointment, clinic, branch, patient, worker, service, status } =
    data;
  return {
    id: appointment.id,
    startsAt: formatDate(appointment.startsAt),
...
export const appointmentCollectionResource = (appointments: Select[]) => {
